...
import { getDriveToken, getDriveFolderId } from "../lib/googleDrive";
      ...
      <main className="flex-1 px-6 py-8 max-w-3xl mx-auto w-full space-y-6">
        <div className="p-4 rounded-xl border border-navy/10 bg-navy/[0.02]">
          ...
          <ul className="font-body text-[11px] text-floral-slate mt-2 space-y-1 list-disc ml-4">
            ...
            <li>Drive token: {(() => { const t = getDriveToken(); if (!t) return "— not connected (tap Connect above)"; return `✓ present, expires in ${Math.round((t.expiresAt - Date.now())/60000)}m`; })()}</li>
            <li>Drive folder: {(() => { const f = getDriveFolderId(); return f ? `✓ ${f.slice(0, 12)}… (My Drive / Wedding Capture / ${slug})` : "— connect to create it"; })()}</li>
          </ul>
